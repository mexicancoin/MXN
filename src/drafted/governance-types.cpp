...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "MXNNetwork") type = MXNNetwork;
    if(strType == "MXNNetworkVariable") type = MXNNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == MXNNetwork) s = "MXNNetwork";
    if(type == MXNNetworkVariable) s = "MXNNetworkVariable";
