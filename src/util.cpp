...
const char * const BITCOIN_CONF_FILENAME = "mxn.conf";
const char * const BITCOIN_PID_FILENAME = "mxnd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "mxn" is a composite category enabling all MXN-related debug output
            if(ptrCategory->count(string("mxn"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MXNCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MXNCore
    // Mac: ~/Library/Application Support/MXNCore
    // Unix: ~/.mxncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MXNCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MXNCore";
#else
    // Unix
    return pathRet / ".mxncore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty mxn.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override mxn.conf
